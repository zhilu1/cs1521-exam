...
Students filterOnWAM(Students ss, float minWAM)
{
    ...
    newss->recs = malloc(newss->nstu * sizeof(sturec_t));
    if(newss->recs == NULL){
        perror("Fail to malloc\n");
        return NULL;    
    }
    ...
    for(int i=0; i < ss->nstu; i++){
        if(ss->recs[i].wam >= minWAM){
            newss->recs[newss->nstu].id = ss->recs[i].id;
            newss->recs[newss->nstu].degree = ss->recs[i].degree;
            newss->recs[newss->nstu].wam = ss->recs[i].wam;
            strcpy(newss->recs[newss->nstu].name, ss->recs[i].name);
            newss->nstu++;
